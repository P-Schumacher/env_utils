...
def update_reference_for_locomotion(reference, motion_file):
    return reference
    if 'walk' in motion_file or 'run' in motion_file:
            horizon = reference['robot'].shape[0]
            ...
            pos_start = reference['robot'][0, 1]
            pos_end = pos_start + horizon * dt * vel
            print(vel)
            print(dt)
            ...
            print(f'{pos_end=}')
            print(f'{horizon=}')
            reference['robot'][:, 1] = reference['robot'][:, 1] + np.linspace(pos_start, pos_end, horizon)
            # reference['robot_vel'][:, 1] = vel
    print(reference['robot_vel'].shape)
    print(reference['robot_vel'])
    return reference
...
class ReferenceMotion():
    ...
    def load(self, reference_data):
        ...
        if 'robot_vel' not in reference.keys():
            reference['robot_vel'] = np.zeros((reference['robot'].shape[0], 37))
        reference = update_reference_for_locomotion(reference, reference_data)
        # build reference
        ref = ReferenceStruct(
            ...
            endeffector= reference['endeffector'] if 'endeffector' in reference.keys() else None,
    ...
    def get_reference(self, time):
        ...
        if self.type == ReferenceType.FIXED:
            ...
            if ind == ind_next:
                ...
                if self.reference['endeffector'] is None:
                    robot_endeffector = None
                else:
                    robot_endeffector = self.reference['endeffector'][ind] if self.robot_horizon>1 else self.reference['endeffector'][0]
                ...
                if self.robot_horizon>1:
                    ...
                    if self.reference['endeffector'] is None:
                        ...
                        robot_endeffector = (1.0 - blend) ** self.reference['endeffector'][ind] + blend * \
                                        self.reference['endeffector'][ind_next]
                else:
                    robot_ref = self.reference['robot'][0]
                    robot_vel_ref = None if self.reference['robot_vel'] is None else self.reference['robot_vel'][0]
                    robot_endeffector = None if self.reference['endeffector'] is None else self.reference['endeffector'][0]
